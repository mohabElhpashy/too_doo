...
const ProductForm = (props) => {
  ...
  const { services, isFetching, isSuccess, refetch } = useFetchSingle(
    `genericDetails/${recordId}`
  );
  const serviceRespose = useFetch("genericDetailsType");
  ...
  const onFinish = async (postObject, recordId) => {
    setLoading(true);
    try {
      await service.post(`/web/genericDetails/update/${recordId}`, postObject);
      setLoading(false);
      openNotificationWithIcon("success");
      refetch();
      history.push("detalies");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              <Flex
                ...
                alignItems="center"
              >
                <h2 className="mb-3">Edit Generic Details</h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
                    onClick={() => history.push("detalies")}
                  >
                    Go Back
                  </Button>
                  <Button
