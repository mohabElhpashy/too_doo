...
const ProductForm = () => {
  ...
  const { services, isSuccess, refetch } = useFetchSingle(
    `tags/service/${ServiceIdConstants.TRIP_ID}/${recordId}`
  );
  ...
  const disabledObjcet = {
    name: "",
    service_id: ServiceIdConstants.TRIP_ID,
  };
  ...
  const onFinish = async (postObject, recordId) => {
    ...
    try {
      await service.put(
        `/web/tags/service/${ServiceIdConstants.TRIP_ID}/${recordId}`,
        postObject
      );
      setLoading(false);
      refetch();
      openNotificationWithIcon("success");
      history.push("tripsTags");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              <Flex
                ...
                alignItems="center"
              >
                <h2 className="mb-3">Edit Trip Tag </h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
                    onClick={() => history.push("tripsTags")}
                  >
                    Go Back
                  </Button>
                  <Button
