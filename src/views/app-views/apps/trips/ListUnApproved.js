...
const ListUnApproved = () => {
  const { services, refetch } = useFetch("trip?unApproved");
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(`/app/apps/trips/editReadTrip?name=view&id=${obj.id}`)
          }
        >
          {name}
        </a>
  ...
  return (
    <>
      <CustomTable
        ...
        noAddOption
        noEditAction
        editEndPoint="trips/readPendingProducts"
        noDeleteAction
      />
      <Modal
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/trip/confirmChanges/${recordId}`,
              postObject
            );
            setStatusVisible(false);
            refetch();
