...
const ProductForm = (props) => {
  ...
  const [postObject, setPostObject] = useState({
    ...
    service_id: ServiceIdConstants.DRESS_ID,
  });
  const { services, isFetching, isSuccess, refetch } = useFetchSingle(
    `tags/service/${ServiceIdConstants.DRESS_ID}/${recordId}`
  );
  ...
  const onFinish = async (postObject, recordId) => {
    ...
    try {
      await service.put(
        `/web/tags/service/${ServiceIdConstants.DRESS_ID}/${recordId}`,
        postObject
      );
      setLoading(false);
      openNotificationWithIcon("success");
      refetch();
      history.push("dressTagss");
    } catch (error) {
      setLoading(false);
    }
  };
            ...
            <div className="container">
              ...
              >
                ...
                <div className="mb-3">
                  <Button
                    className="mr-2"
                    onClick={() => history.push("dressTagss")}
                  >
                    Go Back
                  </Button>
                  <Button
