...
const ListRequests = () => {
  ...
  const { services, isLoading, refetch, error,isSuccess } = useFetch(`dressReservation?itemsPerPage=10&page=${1}`);
...
const [Status_Id,setStatus_Id]=useState([])
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/dressReservation?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/dressReservation?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.data)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
...
// useEffect(async()=>{
//   console.log("allComment",PostComment)
//   try {
//         await service.get(`/web/allComments/reservation`).then(comment=>{
    ...
    <div >
      {/* <CustomTable
        ...
        error={error}
         ...
         searchUrl_order="dressReservation?order_id"
      /> */}
       <CustomTable
        loading={loading}
