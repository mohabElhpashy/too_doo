import {
  ...
  const ListRequests = () => {
    ...
    const { services, isLoading, refetch, error } = useFetch("tripReservation");
    const tripProductRes = useFetch("trip");
  
    const tirpList = tripProductRes.services;
  ...
  const[GetRes_id,setGetRes_id]=useState()
    ...
    const getallComments=async(elm)=>{
      ...
      setGetRes_id(elm.id)
        try {
            await service.get(`/web/allComments/reservation/${6}/${elm.id}`).then(comment=>{
      ...
      },
      {
        ...
        render: (name, obj) => (
          <a
            onClick={() =>
              history.push(
                `/app/apps/trips/editReadTrip?name=view&id=${obj.product_id}`
              )
            }
          >
            {name}
    ...
    return (
      <>
        <CustomTable
          ...
          noEditAction
          noDeleteAction
          editEndPoint="trips/readReservation"
          refetch={refetch}
          isLoading={isLoading}
        />
        <Modal
          ...
          onOk={async () => {
            ...
            try {
              await service.post(
                `web/reservationRequest/${ServiceIdConstants.TRIP_ID}`,
                finalObject
              );
              setStatusVisible(false);
              refetch();
