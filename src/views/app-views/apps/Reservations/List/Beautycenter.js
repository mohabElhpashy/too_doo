...
const ListRequests = () => {
  ...
  const { services, isLoading, refetch, error,isSuccess } = useFetch(
    `beautyCenterServiceReservations?itemsPerPage=10&page=${1}`
  );
  const beautyCenterServiceRes = useFetch("beautyCenterServices");

  const beautyCenterServiceList = beautyCenterServiceRes.services;
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/beautyCenterServiceReservations?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/beautyCenterServiceReservations?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.data)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
  ...
  //   try {
  //     await service.put(`/web/beautyCenterServiceReservations/changeStatus/${id}`, {
  //       status_id: type,
  //     });
  //     refetch();
  ...
  const getallComments=async(elm)=>{
    ...
    setGetRes_id(elm.id)
      try {
          await service.get(`/web/allComments/reservation/${4}/${elm.id}`).then(comment=>{
      ...
      <Menu>
        ...
        <Menu.Item
          onClick={() =>
            handleChange(
              row.id,
              "beautyCenterServiceReservations",
  ...
  const columns = [
    ...
    {
      title: "Beauty Center Service Name",
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/beauty/editBeautyCenterService?name=view&id=${obj.product_id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      {/* <CustomTable
        ...
        noDeleteAction
        noEditAction
        editEndPoint="beauty/readReservation"
        ...
        searchUrl_order="beautyCenterServiceReservations?order_id"
      /> */}
      <CustomTable
        loading={loading}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/reservationRequest/${ServiceIdConstants.BEAUTRY_CENTER_ID}`,
              finalObject
            );
            setStatusVisible(false);
            refetch();
          ...
          {statusVisible.rescdual ? (
            ...
            <>
              <Form.Item
                name="new_product_id"
                required
                label={<span>Beauty Center Product &nbsp;</span>}
              >
                <Select
                  showSearch
