...
const ListRequests = () => {
  ...
  const carProductRes = useFetch("cars");

  const carProductList = carProductRes.services;
  ...
  const [carVarationList, setCarVarationList] = useState([]);
  ...
  const { services, isLoading, refetch, error,isSuccess } = useFetch(`carReservation?itemsPerPage=10&page=${1}`);
const [fortest,setfortest]=useState(false)
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/carReservation?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/carReservation?itemsPerPage=10&page=${page}`).then(respose=>{
  ...
  //   try {
  //     await service.put(`/web/carReservation/changeStatus/${id}`, {
  //       status_id: type,
  //     });
  //     refetch();
  ...
  // console.log(carVarationList, "Car Varation List");
  ...
  const getallComments=async(elm)=>{
    ...
    setGetRes_id(elm.id)
      try {
          await service.get(`/web/allComments/reservation/${1}/${elm.id}`).then(comment=>{
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/cars/editCar?name=view&id=${obj.product_id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      {/* <CustomTable
        ...
        noDeleteAction
         noEditAction
        editEndPoint="cars/readReservation"
        ...
        searchUrl_order="carReservation?order_id"
      /> */}
          <CustomTable
        loading={loading}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/reservationRequest/${ServiceIdConstants.CAR_ID}`,
              finalObject
            );
            setStatusVisible(false);
            refetch();
          ...
          {statusVisible.rescdual ? (
            ...
            <>
                ...
                <Select
                  ...
                  onChange={async (e) => {
                    try {
                      ...
                      const carRes = await service
                        .get(`web/carVariation/${e}`)
                        .then((res) => res?.record);
                      setCarVarationList(carRes);
                    } catch (error) {}
                  }}
              ...
              <Form.Item
                ...
                required
                label={<span>Car Varation &nbsp;</span>}
              >
                <Select
                  showSearch
                  placeholder="Select a Number of Hours"
                  optionFilterProp="children"
                  disabled={carVarationList.length === 0}
                  onChange={(e) =>
    ...
    }):<div style={{display:"flex",justifyContent:"center",alignItems:"center"}}><h1>No Comments Added</h1></div>}
