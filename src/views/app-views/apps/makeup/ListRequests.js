...
const ListRequests = () => {
  ...
  const { services, isLoading, refetch,isSuccess,error } = useFetch(
    `makeupServiceReservations?itemsPerPage=10&page=${1}`
  );
  const makeupRes = useFetch("makeups");

  const makeupProductList = makeupRes.services;
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/makeupServiceReservations?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/makeupServiceReservations?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.data)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
  ...
  //   try {
  //     await service.put(`/web/makeupServiceReservations/changeStatus/${id}`, {
  //       status_id: type,
  //     });
  //     refetch();
  ...
  const getallComments=async(elm)=>{
    ...
    setGetRes_id(elm.id)
      try {
          await service.get(`/web/allComments/reservation/${8}/${elm.id}`).then(comment=>{
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/makeup/editmakeupService?name=view&id=${obj.product_id}`
            )
          }
        >
          {name}
    ...
    <>
      {/* <CustomTable
        pageTitle="List of Reservations"
         coloumRender={columns}
          editEndPoint="makeup/readReservation"
        noAddOption
        noDeleteAction
        noEditAction
        error={error}
         ...
         searchUrl_order="makeupServiceReservations?order_id"
      /> */}
       <CustomTable
        loading={loading}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/reservationRequest/${ServiceIdConstants.MAKEUP_ID}`,
              finalObject
            );
            setStatusVisible(false);
            refetch();
          ...
          {statusVisible.rescdual ? (
            ...
            <>
              <Form.Item
                name="new_product_id"
                required
                label={<span>MakeUp Product &nbsp;</span>}
              >
                <Select
                  showSearch
                  placeholder="Select a CheckUp"
