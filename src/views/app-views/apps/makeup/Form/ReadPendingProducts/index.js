...
import MakeUpDetails from "../EditForm/EditMakeUpServiceForm/MakeUpDetails";
...
const ProductForm = () => {
  ...
  // Store List
  const clinicRes = useFetch("makeups");
  const tagsRes = useFetch("tags");
  const tagList = tagsRes.services;
  const clinicList = clinicRes.services;
  const clinicIsSuccess = clinicRes.isSuccess;
  const recordId = new URLSearchParams(location.search).get("id");
  const genericDetailes = useFetch(
    `genericDetailsProducts/${serviceId.MAKEUP_ID}`
  );
  ...
  const { services, isSuccess } = useFetchSingle(
    `makeupServices/changes/${recordId}`
  );
