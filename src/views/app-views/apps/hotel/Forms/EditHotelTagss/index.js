...
const ProductForm = (props) => {
  ...
  const [postObject, setPostObject] = useState({
    ...
    service_id: ServiceIdConstants.CAR_ID,
  });
  const { services, isFetching, isSuccess, refetch } = useFetchSingle(
    `tags/service/${ServiceIdConstants.HOTEL_ID}/${recordId}`
  );
  ...
  const disabledObjcet = {
    name: "",
    service_id: ServiceIdConstants.HOTEL_ID,
  };
  ...
  const onFinish = async (postObject, recordId) => {
    ...
    try {
      await service.put(
        `/web/tags/service/${ServiceIdConstants.HOTEL_ID}/${recordId}`,
        postObject
      );
      refetch();
      setLoading(false);
      openNotificationWithIcon("success");
      history.push("hotelTagss");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              <Flex
                ...
                alignItems="center"
              >
                <h2 className="mb-3">Edit Hotel Tag </h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
                    onClick={() => history.push("hotelTagss")}
                  >
                    Go Back
                  </Button>
                  <Button
