...
const ListRequests = () => {
  ...
  const { services, isLoading, refetch, error,isSuccess } = useFetch(
    `weddingHallReservation?itemsPerPage=10&page=${1}`
  );
  const weddingHallListRes = useFetch("weddingHall");

  const weddingHallList = weddingHallListRes.services;
  ...
  const [weddingHallVaration, setweddingHallVaration] = useState([]);
  ...
  const [loading, setloading] = useState(false);  useEffect(async()=>{
    try {
      Fetch_record()
      } catch (error) {
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/weddingHallReservation?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/weddingHallReservation?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.data)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
  ...
  const getallComments=async(elm)=>{
    ...
    setGetRes_id(elm.id)
      try {
          await service.get(`/web/allComments/reservation/${2}/${elm.id}`).then(comment=>{
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/hotel/editWeddinghall?name=view&id=${obj.product_id}`
            )
          }
        >
          {name}
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/hotel/veHotelForm?name=view&id=${obj.store_id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      {/* <CustomTable
        ...
        noEditAction
        editEndPoint="hotel/readReservation"
        coloumRender={columns}
         ...
         searchUrl_order="weddingHallReservation?order_id"
        // searchUrl="weddingHallReservation?order_id"
      /> */}
        <CustomTable
        loading={loading}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/reservationRequest/${ServiceIdConstants.HOTEL_ID}`,
              finalObject
            );
            setStatusVisible(false);
            refetch();
          ...
          {statusVisible.rescdual ? (
            ...
            <>
                ...
                <Select
                  ...
                  onChange={async (e) => {
                    try {
                      ...
                      const weddingHallRes = await service
                        .get(`web/WeddingHallVariation/${e}`)
                        .then((res) => res?.record);
                      setweddingHallVaration(weddingHallRes);
                    } catch (error) {}
                  }}
              ...
              <Form.Item
                ...
                required
                label={<span>Wedding Hall Varation &nbsp;</span>}
              >
                <Select
                  showSearch
                  placeholder="Select a Varation"
                  optionFilterProp="children"
                  disabled={weddingHallVaration.length === 0}
                  onChange={(e) =>
                ...
                >
                  {weddingHallVaration?.map((element) => (
                    <Select.Option key={element.id} value={element.id}>
                      {element?.variation_name_en}
                    </Select.Option>
                  ))}
                </Select>
              </Form.Item>
