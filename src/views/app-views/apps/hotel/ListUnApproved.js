...
const ListUnApproved = () => {
  const { services, isLoading, refetch } = useFetch("weddingHall?unApproved");
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/hotel/editWeddinghall?name=view&id=${obj.id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      <CustomTable
        ...
        noAddOption
        noEditAction
        editEndPoint="hotel/readPendingProducts"
        noDeleteAction
      />
      <Modal
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/weddingHall/confirmChanges/${recordId}`,
              postObject
            );
            setStatusVisible(false);
            refetch();
