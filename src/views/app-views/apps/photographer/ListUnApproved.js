...
const ListUnApproved = () => {
  const { services, refetch } = useFetch("photosession?unApproved");
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/photographer/editphotoSession?name=view&id=${obj.id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      <CustomTable
        ...
        noAddOption
        noEditAction
        editEndPoint="photographer/readPendingProducts"
        noDeleteAction
      />
      <Modal
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/photosession/confirmChanges/${recordId}`,
              postObject
            );
            setStatusVisible(false);
            refetch();
