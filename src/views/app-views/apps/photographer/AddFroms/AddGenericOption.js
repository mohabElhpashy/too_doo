...
const AddGenericOption = () => {
  ...
  const handelSubmit = async (postObject) => {
    try {
      if (initalState === emptyState) {
        await service.post("/web/photoSessionGenericOption", postObject);
      } else {
        await service.put(
          `/web/photoSessionGenericOption/${postObject.id}`,
          postObject
        );
      }
  ...
  return (
    <>
      {isSuccess || error ? (
        <CustomSubmittion
          samePage="photographer/AddGenericOption"
          tablePage="photographer/GenericOptions"
