...
const AddPhotoGrapher = () => {
  ...
  const handelSubmit = async (postObject) => {
    try {
      if (initalState === emptyState) {
        ...
        await service.put(`/web/photosession/${postObject.id}`, postObject);
      }
      setLoading(false);
      setSuccess(true);
    } catch (error) {
  ...
  return (
    <>
      {isSuccess || error ? (
        <CustomSubmittion
          samePage="photographer/AddPhotoGrapher"
          tablePage="photographer/photoGrapher"
          ...
          >
            ...
            <Form.Item name={"seller_id"} label="Seller Id">
              <Input
                ...
                placeholder="Please enter your PhotoGrapher "
