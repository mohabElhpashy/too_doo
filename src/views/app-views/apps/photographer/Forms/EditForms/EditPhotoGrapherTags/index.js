...
const ProductForm = () => {
  ...
  const { services, isSuccess, refetch } = useFetchSingle(
    `tags/service/${ServiceIdConstants.Photographers_Id}/${recordId}`
  );
  ...
  const onFinish = async (postObject, recordId) => {
    ...
    try {
      await service.put(
        `/web/tags/service/${ServiceIdConstants.Photographers_Id}/${recordId}`,
        postObject
      );
      setLoading(false);
      refetch();
      openNotificationWithIcon("success");
      history.push("photoSessionTags");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              <Flex
                ...
                alignItems="center"
              >
                <h2 className="mb-3">Edit Photo Session Tags</h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
