...
import PhotoDetails from "../EditForms/EditPhotoSessionService/PhotoDetails";
...
const ProductForm = () => {
  ...
  // Store List
  const photoGrapherRes = useFetch("photographer");
  const tagsRes = useFetch("tags");
  const tagList = tagsRes.services;
  const photoGrapherList = photoGrapherRes.services;
  const photoIsSuccess = photoGrapherRes.isSuccess;
  const recordId = new URLSearchParams(location.search).get("id");
  const genericDetailes = useFetch(
    `genericDetailsProducts/${serviceId.Photographers_Id}`
  );
