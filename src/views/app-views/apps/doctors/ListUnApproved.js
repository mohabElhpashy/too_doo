...
const ListUnApproved = () => {
  const { services, isLoading, refetch } = useFetch("checkups?unApproved");
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/doctors/eeditCheckUp?name=view&id=${obj.id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      <CustomTable
        ...
        noAddOption
        noEditAction
        editEndPoint="doctors/readPendingProducts"
        noDeleteAction
      />
      <Modal
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/checkups/confirmChanges/${recordId}`,
              postObject
            );
            setStatusVisible(false);
            refetch();
