...
const ListRequests = () => {
  ...
  const { services, isLoading, refetch, error,isSuccess } =
    useFetch(`checkupReservation?itemsPerPage=10&page=${1}`);
  const checkUpsRes = useFetch("checkups");

  const checkUpProductList = checkUpsRes.services;
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/checkupReservation?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/checkupReservation?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.data)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
  ...
  //   try {
  //     await service.put(`/web/checkupReservation/changeStatus/${id}`, {
  //       status_id: type,
  //     });
  //     refetch();
  ...
  console.log(currentObject, "This is The current Head");
  ...
  const getallComments=async(elm)=>{
    ...
    setGetRes_id(elm.id)
      try {
          await service.get(`/web/allComments/reservation/${3}/${elm.id}`).then(comment=>{
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/doctors/eeditCheckUp?name=view&id=${obj.product_id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      {/* <CustomTable
        ...
        editEndPoint="doctors/readReservation"
        ...
        searchUrl_order="checkupReservation?order_id"
      /> */}
      <CustomTable
        loading={loading}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/reservationRequest/${ServiceIdConstants.DOCTOR_ID}`,
              finalObject
            );
            setStatusVisible(false);
            refetch();
          ...
          {statusVisible.rescdual ? (
            ...
            <>
              ...
              >
                ...
                >
                  {checkUpProductList?.map((element) => (
