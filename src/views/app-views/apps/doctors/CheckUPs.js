...
const CheckUps = () => {
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/checkups?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/checkups?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.records)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
