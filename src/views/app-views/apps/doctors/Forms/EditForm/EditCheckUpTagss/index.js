...
const ProductForm = (props) => {
  ...
  const { services, isFetching, isSuccess, refetch } = useFetchSingle(
    `tags/service/${ServiceIdConstants.DOCTOR_ID}/${recordId}`
  );
  ...
  const disabledObjcet = {
    name: "",
    service_id: ServiceIdConstants.DOCTOR_ID,
  };
  ...
  const onFinish = async (postObject, recordId) => {
    ...
    try {
      await service.put(
        `/web/tags/service/${ServiceIdConstants.DOCTOR_ID}/${recordId}`,
        postObject
      );
      setLoading(false);
      refetch();
      openNotificationWithIcon("success");
      history.push("checkupTag");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              <Flex
                ...
                alignItems="center"
              >
                <h2 className="mb-3">Edit CheckUp Tag </h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
