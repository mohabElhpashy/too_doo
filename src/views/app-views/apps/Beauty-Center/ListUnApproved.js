...
const ListUnApproved = () => {
  const { services, isLoading, refetch } = useFetch(
    "beautyCenterServices?unApproved"
  );
    ...
    },
    {
      ...
      render: (name, obj) => (
        <a
          onClick={() =>
            history.push(
              `/app/apps/beauty/editBeautyCenterService?name=view&id=${obj.id}`
            )
          }
        >
          {name}
  ...
  return (
    <>
      <CustomTable
        ...
        noAddOption
        noEditAction
        editEndPoint="beauty/readPendingProducts"
        noDeleteAction
      />
      <Modal
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            await service.post(
              `web/beautyCenterServices/confirmChanges/${recordId}`,
              postObject
            );
            setStatusVisible(false);
            refetch();
