...
const ProductForm = (props) => {
  ...
  const onFinish = async (postObject, recordId) => {
    setLoading(true);
    try {
      await service.put(`/web/beautyCenters/${recordId}`, postObject);
      setLoading(false);
      openNotificationWithIcon("success");
      refetch();
      history.push("beautyCenter");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              ...
              >
                <h2 className="mb-3">
                  {actionType === "view"
                    ? "Read Beauty Center Store"
                    : "Edit Beauty Center Store"}
                </h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
