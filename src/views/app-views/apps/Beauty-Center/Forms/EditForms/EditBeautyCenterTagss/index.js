...
const ProductForm = (props) => {
  ...
  const [postObject, setPostObject] = useState({
    ...
    service_id: ServiceIdConstants.BEAUTRY_CENTER_ID,
  });
  const { services, isFetching, isSuccess, refetch } = useFetchSingle(
    `tags/service/${ServiceIdConstants.BEAUTRY_CENTER_ID}/${recordId}`
  );
  ...
  const onFinish = async (postObject, recordId) => {
    ...
    try {
      await service.put(
        `/web/tags/service/${ServiceIdConstants.BEAUTRY_CENTER_ID}/${recordId}`,
        postObject
      );
      setLoading(false);
      refetch();
      openNotificationWithIcon("success");
      history.push("beautyCenterTag");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              <Flex
                ...
                alignItems="center"
              >
                <h2 className="mb-3">Edit Beauty Center Tag </h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
