...
const ProductForm = (props) => {
  ...
  const { services, isLoading, isSuccess } = useFetchSingle(
    `offlineReservation/single/4/${recordId}`
  );
  //Handle Notification
  ...
  useEffect(() => {
    if (isSuccess) {
      (async () => {
        try {
          await service.get(`/web/products/4/${services?.seller_id}`);
  ...
  useEffect(() => {
    (async () => {
      const data = await service.get("/web/stores/4");
  ...
  const onFinish = async (postObject, recordId) => {
    setLoading(true);
    try {
      await service.post(`/web/offlineReservation/${recordId}`, postObject);
      setLoading(false);
