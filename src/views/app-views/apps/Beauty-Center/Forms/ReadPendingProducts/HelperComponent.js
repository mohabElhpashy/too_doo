...
const HelperComponent = (props) => {
  ...
  const filteredTags = props?.tagsList?.filter(
    (element) => element.service_id === ServiceIdConstants.BEAUTRY_CENTER_ID
  );
  useEffect(() => {
    ...
    <Row gutter={16}>
      <Col xs={24} sm={24} md={17}>
        <Card title="Basic Info" style={{ minHeight: 525 }}>
          ...
          {props.postObject.beauty_center_id && (
            <Form.Item
              name="beauty_center_id"
              required
              label={<span>Beauty Center Name&nbsp;</span>}
            >
              <Select
                showSearch
                name="beauty_center_id"
                disabled={true}
                defaultValue={props?.postObject?.beauty_center_id}
                placeholder="Select a Beauty Center Name"
