...
import BeautyCenterDetails from "../EditForms/EditBeautyCenterService/BeautyCenterDetails";
...
const ProductForm = () => {
  ...
  // Store List
  const beautyCenterRes = useFetch("beautyCenters");
  const tagsRes = useFetch("tags");
  const tagList = tagsRes.services;
  const beautyCenterList = beautyCenterRes.services;
  const beautyCenterIsSuccess = beautyCenterRes.isSuccess;
  ...
  const { services, isFetching, isSuccess } = useFetchSingle(
    `beautyCenterServices/changes/${recordId}`
  );
  ...
  return (
    <>
      {isSuccess && beautyCenterIsSuccess ? (
        <Form
          layout="vertical"
          form={form}
