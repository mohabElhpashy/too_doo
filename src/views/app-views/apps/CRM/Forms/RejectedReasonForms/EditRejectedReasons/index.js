...
const ProductForm = () => {
  ...
  const onFinish = async (postObject, recordId) => {
    setLoading(true);
    try {
      await service.put(`/web/reject/reasons/${recordId}`, postObject);
      setLoading(false);
      refetch();
      openNotificationWithIcon("success");
      history.push("rejectResons");
    } catch (error) {
      setLoading(false);
    }
  };
          ...
          <PageHeaderAlt className="border-bottom" overlap>
            <div className="container">
              ...
              >
                <h2 className="mb-3">
                  {checkViewMode === "view"
                    ? "Read Rejected Reason"
                    : "Edit Rejected Reason"}
                </h2>
                <div className="mb-3">
                  <Button
                    className="mr-2"
                    onClick={() => history.push("rejectResons")}
                  >
                    Go Back
                  </Button>
                  <Button
