...
const ProductForm = (props) => {
  ...
  useEffect(() => {
    if (isSuccess) {
      (async () => {
        try {
          await service.get(`/web/products/1/${services?.seller_id}`);
  ...
  useEffect(() => {
    (async () => {
      const data = await service.get("/web/stores/1");
