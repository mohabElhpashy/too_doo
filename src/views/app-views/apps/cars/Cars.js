import { Badge, message, Modal, Tag, Upload,Comment,Form,Input,Button } from "antd";
...
const Cars = () => {
  ...
  const Fetch_record=async(page)=>{
    ...
    if(page==undefined)
    {
      const Records= await service.get(`/web/cars?itemsPerPage=10&page=${1}`).then(respose=>{
      ...
      try {
        ...
        const Records= await service.get(`/web/cars?itemsPerPage=10&page=${page}`).then(respose=>{
          setCurrentList(respose.records)
          settotalPages(respose.paginationInfo.totalRecords)
          setloading(false)
        }) ;
  ...
  const Fetch_all_record=async()=>{
    ...
    setCurrentList()
      const Post=await service.get(`web/cars/searchPrice?priceFrom=${from}&priceTo=${to}`).then(
        res=>{
