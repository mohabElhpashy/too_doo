...
import CarsDetails from "../Form/EditForm/EditCar/CarsDetails";
import CarVariationField from "../Form/EditForm/EditCar/CarVariationField";
...
const ProductForm = () => {
  ...
  // Store List
  const carAgancyRes = useFetch("agencies");
  const tagsRes = useFetch("tags");
  const tagList = tagsRes.services;
  const carAgancyList = carAgancyRes.services;
  const dressSoreListSuccess = carAgancyRes.isSuccess;
  const recordId = new URLSearchParams(location.search).get("id");
  const genericDetailes = useFetch(
    `genericDetailsProducts/${serviceId.CAR_ID}`
  );
  ...
  const TEST_FINAL_Form = [
    {
      variation: [{ id: 0, name: "rent" }],
      details: [{ id: 1, generic_detals: "Stars" }],
      description_en: "Lorem Konafa be el manga",
      name_en: "Mohamed Abbas",
      name_ar: "السلعاوي",
    },
  ];
