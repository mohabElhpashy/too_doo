...
const HelperComponent = (props) => {
  ...
  const filteredTags = props?.tagsList?.filter(
    (element) => element.service_id === ServiceIdConstants.CAR_ID
  );
  useEffect(() => {
    ...
    <Row gutter={16}>
      <Col xs={24} sm={24} md={17}>
        <Card title="Basic Info" style={{ minHeight: 525 }}>
          ...
          {props.postObject.agency_id && (
            <Form.Item
              name="agency_id"
              required
              label={<span>Agancy Name&nbsp;</span>}
            >
              <Select
                showSearch
                name="agency_id"
              ...
              <>
                ...
                >
                  <Input
                    name="number_of_hours"
                    disabled={true}
                    addonAfter={element.number_of_hours == 1 ? "Hour" : "Hours"}
                    onPressEnter={(e) => e.preventDefault()}
                    defaultValue={element.number_of_hours}
                    // placeholder="Please enter Product Name in English"
                  />
                </Form.Item>
                <Form.Item
